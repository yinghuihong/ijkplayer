...
static void control_queue_duration(FFPlayer *ffp, VideoState *is) {
    ...
    printf("________ max_cached_duration2 : %d\n", is->max_cached_duration);
    if (is->video_st) {
        return control_video_queue_duration(ffp, is);
    }
}
//====================================== edit yinghuihong end ==========================================


/* this thread gets the stream from the disk or the network */
static int read_thread(void *arg)
{
    printf("_______ read_thread\n");
    FFPlayer *ffp = arg;
    VideoState *is = ffp->is;
...
//================================== edit by yinghuihong start =====================================
//    is->realtime = is_realtime(ic);
    is->realtime = 0;
//    is->max_cached_duration = 1000;
    AVDictionaryEntry *e = av_dict_get(ffp->player_opts, "max_cached_duration", NULL, 200);
    if (e) {
...
//      of the seek_pos/seek_rel variables
        ...
        // edit by yinghuihong 每次读取一个pkt，都去判断处理
        // TODO:优化，不用每次都调用
        printf("________ max_cached_duration : %d\n", is->max_cached_duration);
        if (is->max_cached_duration > 0) {
